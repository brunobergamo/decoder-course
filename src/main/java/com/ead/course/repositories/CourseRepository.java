...
public interface CourseRepository extends JpaRepository<CourseModel, UUID>, JpaSpecificationExecutor<CourseModel> {
    ...
    @Query(value = "select case when count(tcu) > 0 THEN true ELSE false END from tb_courses_users tcu WHERE tcu.course_id= :courseId and " +
            "tcu.user_id = :userId ",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "delete tb_courses_users where course_id = courseId); ",nativeQuery = true)
    void deleteCourseUserByCourse(@Param("courseId") UUID courseId );

    @Modifying
    @Query(value = "delete tb_courses_users where user_id = userId); ",nativeQuery = true)
